...
#define SET_AND_RETURN_ERROR(err) \
  do { \
    set_error(err); \
    return err; \
  } while(0)
...
VkBool32 debug_callback(
  ...
  // if (err_data && err_data->cb) {
    ...
    std::cout << "severity: " << sev << " type: " << ty <<
        "msg: " << msg_buf << std::endl;
    // err_data->cb({
    //   .severity = sev,
    //   .type = ty,
...
DeviceBuilder* DeviceBuilder::app_name(const char* app_name) {
  app_name_ = app_name;
  return this;
}
...
DeviceError DeviceBuilder::check_validation_available_if_needed() const {
  ...
  for (const char* name : kValidationLayers) {
    ...
    for (const auto& prop : layers) {
      if (strncmp(name, prop.layerName, strlen(name)) == 0) {
        found = true;
        break;
      }
    }
    if (!found)
      SET_AND_RETURN_ERROR(DeviceError::kValidationLayerNotAvailable);
  }
  return DeviceError::kNone;
}
...
VkApplicationInfo DeviceBuilder::build_app_info() {
  return {
    ...
    .pApplicationName = app_name_,
    .applicationVersion =
        VK_MAKE_VERSION(version_.major, version_.minor, version_.patch),
    .pEngineName = kEngineName,
    .engineVersion = VK_MAKE_VERSION(kEngineMajor, kEngineMinor, kEnginePatch),
...
DeviceError DeviceBuilder::setup_debug_handler_if_needed(Device* d,
  ...
  if (func == nullptr)
    SET_AND_RETURN_ERROR(DeviceError::kErrorExtensionNotPresent);

  auto res = func(d->instance_, debug_create_info, nullptr, &d->debug_handler_);
  if (res != VK_SUCCESS)
    SET_AND_RETURN_ERROR(DeviceError::kDebugHandlerCreateFailed);
...
DeviceError DeviceBuilder::pick_physical_device(Device* d) {
  uint32_t count = 0;
  vkEnumeratePhysicalDevices(d->instance_, &count, nullptr);
  if (count == 0)
    SET_AND_RETURN_ERROR(DeviceError::kNoSupportedGPUsFound);
  ...
  if (d->physical_device_.device == VK_NULL_HANDLE)
    SET_AND_RETURN_ERROR(DeviceError::kNoSuitableGPUFound);

  return DeviceError::kNone;
}

std::unique_ptr<Device> DeviceBuilder::build() {
  CHECK_ERROR(check_validation_available_if_needed());
  ...
  if (res != VK_SUCCESS) {
    set_error(DeviceError::kInstanceCreateFailed);
    return nullptr;
  }

  CHECK_ERROR(setup_debug_handler_if_needed(d, &debug_create_info));
  CHECK_ERROR(pick_physical_device(d));
